...
GerCol<Single, Lower, Diag, Upper, lhs_t, rhs_1_t, rhs_2_t>::eval(
  ...
  index_t subSz = ndItem.get_sub_group().get_group_linear_range();
  ...
  const index_t id_row0 = frs_row + localid;//sublid;
...
//    for (index_t sub_id_col = 0; sub_id_col < subSz; sub_id_col++) {
      if(id_row0 < dimR && id_col/*0 + sub_id_col*/ < dimC)
        lhs_.eval(id_row0, frs_col + id_col/*0 + sub_id_col*/) += scal_rhs_1 * //cl::sycl::group_broadcast(ndItem.get_sub_group(), rhs_2, sub_id_col);
rhs_2_.eval(frs_col + id_col/*0 + sub_id_col*/);
//    }
  }
