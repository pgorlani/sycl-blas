...
//        _off0 += local_range;
  ...
  for (index_t _it = 0; _it < local_range; ++_it) {
    ...
    if (!is_unitdiag && (l_idx == l_diag))
      l_x[_idx]=sycl::native::divide(l_x[_idx], l_x[local_range*(1 + l_diag) + l_diag]);

    sycl::atomic_fence(sycl::memory_order::seq_cst, sycl::memory_scope::sub_group);

    l_x[_idx] -= l_x[local_range*(1 + l_diag) + _idx] * l_x[l_diag] * ((((_idx > l_diag) && is_forward) ||
        ((_idx < l_diag) && !is_forward)) ?
       value_t(1) : value_t(0));
