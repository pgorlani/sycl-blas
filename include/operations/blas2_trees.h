...
struct Tbsv {
  ...
  Tbsv(lhs_t &_l, matrix_t &_matrix, index_t &_k, vector_t &_vector);
  index_t get_size() const;
  bool valid_thread(cl::sycl::nd_item<1> ndItem) const;
  value_t eval(index_t i);
  value_t eval(cl::sycl::nd_item<1> ndItem);
...
template <uint32_t local_range, bool is_upper, bool is_transposed, bool is_unit,
          typename lhs_t, typename matrix_t, typename vector_t>
Tbsv<lhs_t, matrix_t, vector_t, local_range, is_upper, is_transposed, is_unit>
make_tbsv(lhs_t &lhs_, matrix_t &matrix_, typename vector_t::index_t k_,
          vector_t &vector_) {
  return Tbsv<lhs_t, matrix_t, vector_t, local_range, is_upper, is_transposed,
              is_unit>(lhs_, matrix_, k_, vector_);
}
